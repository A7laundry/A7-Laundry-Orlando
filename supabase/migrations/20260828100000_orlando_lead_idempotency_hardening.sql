-- A7 Laundry Orlando — reject cross-customer reuse of a lead idempotency key.
...
begin
  ...
  if v_lead.id is null then
    ...
    select * into v_event from public.a7_orlando_order_events where idempotency_key = 'event:' || p_idempotency_key;
    if v_lead.id is null
      or v_event.event_name <> 'generate_lead'
      or v_event.event_id <> p_event_id
      or v_event.lead_id <> v_lead.id
      or v_lead.lead_origin <> p_lead_origin
