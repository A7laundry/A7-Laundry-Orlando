...
begin
  ...
  -- Resolve an exact prior write before evaluating mutable workflow state. A network retry must remain
  -- idempotent even if processing advanced immediately after the original weight was committed.
...
-- Service-role-only release probe. It exercises the real W1C-A write twice and
-- removes every synthetic row before the transaction commits.
