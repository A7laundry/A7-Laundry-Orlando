-- A7 Orlando OS W1B — compatibility fix for pickup scheduling.
-- `pickup_scheduled` is an operational state, not a canonical analytics lifecycle event.
...
declare
  ...
  v_weight numeric;
begin
  if p_event_name <> 'pickup_scheduled'
    and exists (select 1 from public.a7_orlando_order_events where idempotency_key = p_idempotency_key) then
...
comment on function public.a7_orlando_record_transition(uuid,text,text,text,text,jsonb,timestamptz)
  is 'Canonical lifecycle transition writer with W1B pickup-scheduling compatibility; scheduling is recorded only in the W1B operational ledger.';
