...
- [x] `npm run lint`, `npm run typecheck`, `npm test`, `npm run build` and `git diff --check` pass.
- [x] No deploy, campaign publication or production mutation was performed during implementation; production authorization was provided separately on 2026-08-16.
...
- `npm test`: passed after the attribution adapter contract was reconciled to `shadow_ephemeral`.
...
- No deploy, ad-platform mutation or production operation was performed.
