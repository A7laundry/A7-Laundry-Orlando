...
module.exports = async function handler(req, res) {
  ...
  if (!body || body.confirm !== 'W3D_TRANSACTIONAL_SMOKE') {
  ...
  try {
    const result = await systemW3dSmokeService().run(actor, requestId);
    ...
    console.error('system_w3d_smoke_failed', { name:error?.name || 'Error' });
    return json(res, 500, { ok:false, code:'unexpected_error', error:'W3-D smoke failed.' });
  }
};
