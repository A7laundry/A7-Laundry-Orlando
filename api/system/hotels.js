...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (body.action === 'list') {
      const rows = await hotels.list({ query:body.query, include_inactive:actor.role === 'owner' && body.include_inactive, limit:body.limit });
      return json(res, 200, { ok:true, hotels:rows });
    }
    if (body.action === 'save') {
      if (actor.role !== 'owner') return json(res, 403, { ok:false, code:'forbidden', error:'Owner authorization is required.' });
