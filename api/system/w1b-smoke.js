...
module.exports = async function handler(req, res) {
  ...
  if (!body || body.confirm !== 'W1B_TRANSACTIONAL_SMOKE') {
  ...
  try {
    const result = await systemW1bSmokeService().run(actor, requestId);
    ...
    console.error('system_w1b_smoke_failed', { name:error?.name || 'Error' });
    return json(res, 500, { ok:false, code:'unexpected_error', error:'W1B smoke failed.' });
  }
};
