...
class MemoryOperationalStore {
  ...
  async upsertSystemDriver(input) {
    ...
    if (priorEvent) {
      const driver = this.systemDrivers.get(priorEvent.driver_id);
      if (!driver || priorEvent.action !== 'driver_saved') {
        throw new InvalidTransitionError('Idempotency key conflicts with another driver action.');
      }
      return { duplicate:true, driver:clone(driver) };
    }
    ...
    const event = { id:crypto.randomUUID(), driver_id:driver.id, action:'driver_saved', actor_id:input.actor_id,
      actor_role:input.actor_role, idempotency_key:input.idempotency_key, occurred_at:when };
  ...
  async assignSystemDriver(input) {
    ...
    if (prior) {
      if (prior.order_id !== order.id || prior.driver_id !== driver.id || prior.leg !== input.leg) {
  ...
  async recordSystemManualPayment(input) {
    ...
    if (Number(order.service_amount) !== Number(input.amount)) {
      throw new InvalidTransitionError('Payment amount must match the current invoice service amount.');
    }
    ...
    const payment = { id:crypto.randomUUID(), order_id:order.id, order_number:order.order_number,
      method:input.method, amount:input.amount, currency:'USD', paid_at:input.paid_at,
    ...
    order.paid_at = payment.paid_at; order.currency = 'USD'; order.tip_amount = 0;
...
class SupabaseOperationalStore {
  ...
  async recordSystemManualPayment(input) {
    return this.rpc('a7_orlando_record_manual_payment', {
      p_order_number:input.order_number, p_method:input.method, p_amount:input.amount,
      p_paid_at:input.paid_at, p_note:input.note || null, p_actor_id:input.actor_id,
  ...
  async getSystemOperationalSnapshot() {
    return this.rpc('a7_orlando_operational_cycle_snapshot', {});
  }
  ...
  async transitionSystemOperationalOrder(input) {
    ...
    return this.rpc('a7_orlando_operational_cycle_transition', {
