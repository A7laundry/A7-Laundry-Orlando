...
function canonicalServiceCode(serviceTier) {
  const tier = String(serviceTier || '').trim().toLowerCase();
  if (tier === 'express') return 'EXPRESS_8H';
  if (tier === 'normal' || tier === 'standard') return 'STANDARD_24H';
  return null;
}
...
class MemoryOperationalStore {
  constructor() {
    ...
    this.hotels = new Map();
    this.hotelEvents = new Map();
    this.systemDrivers = new Map();
    this.systemDriverAssignments = new Map();
    this.systemManualPayments = new Map();
    this.systemPaymentLinks = new Map();
  ...
  async listSystemHotels(input = {}) {
    const query = String(input.query || '').trim().toLowerCase();
    const rows = [...this.hotels.values()].filter((hotel) => (input.include_inactive || hotel.active)
      && (!query || [hotel.canonical_name, hotel.address_line, hotel.region, ...(hotel.aliases || [])]
        .some((value) => String(value || '').toLowerCase().includes(query))));
    return rows.sort((a, b) => a.canonical_name.localeCompare(b.canonical_name))
      .slice(0, Math.max(1, Math.min(Number(input.limit) || 100, 200))).map((hotel) => {
        const orders = [...this.orders.values()].filter((order) => order.hotel_id === hotel.id
          && order.order_status !== 'cancelled' && !isQaOrder(order, this.leads.get(order.lead_id), this.customers.get(order.customer_id)));
        const confirmed = orders.map((order) => confirmedServiceRevenue(order, this.payments, false))
          .filter((value) => value != null);
        return clone({ ...hotel, hotel_id:hotel.id, order_count:orders.length,
          confirmed_order_count:confirmed.length,
          confirmed_service_revenue:confirmed.reduce((sum, value) => sum + value, 0),
          average_confirmed_ticket:confirmed.length ? confirmed.reduce((sum, value) => sum + value, 0) / confirmed.length : null,
          normal_orders:orders.filter((order) => order.service_tier === 'normal').length,
          express_orders:orders.filter((order) => order.service_tier === 'express').length,
          new_customer_orders:orders.filter((order) => !order.is_repeat_customer).length,
          repeat_customer_orders:orders.filter((order) => order.is_repeat_customer).length,
          last_service_at:orders.map((order) => order.accepted_at).filter(Boolean).sort().at(-1) || null,
          currency:'USD' });
      });
  }

  async listSystemDrivers(input = {}) {
    return [...this.systemDrivers.values()]
      .filter((row) => input.include_inactive || row.active)
      .sort((left, right) => left.full_name.localeCompare(right.full_name))
      .map(clone);
  }

  async upsertSystemDriver(input) {
    const priorEvent = [...this.operationalEvents.values()].find((row) => row.idempotency_key === input.idempotency_key);
    if (priorEvent) {
      if (priorEvent.action !== 'driver_saved'
        || priorEvent.request?.driver_id !== (input.driver_id || null)
        || priorEvent.request?.full_name !== input.full_name
        || priorEvent.request?.phone !== input.phone
        || priorEvent.request?.active !== input.active
        || priorEvent.actor_id !== input.actor_id
        || priorEvent.actor_role !== input.actor_role
        || !priorEvent.result) {
        throw new InvalidTransitionError('Idempotency key conflicts with another driver action.');
      }
      return { duplicate:true, driver:clone(priorEvent.result) };
    }
    let driver = input.driver_id ? this.systemDrivers.get(input.driver_id) : null;
    if (input.driver_id && !driver) throw new InvalidTransitionError('Driver not found.');
    const duplicatePhone = [...this.systemDrivers.values()].find((row) => row.phone === input.phone && row.id !== input.driver_id);
    if (duplicatePhone) throw new InvalidTransitionError('Driver phone is already registered.');
    const when = iso(input.occurred_at);
    if (!driver) {
      driver = { id:crypto.randomUUID(), full_name:input.full_name, phone:input.phone, active:input.active,
        created_at:when, updated_at:when };
      this.systemDrivers.set(driver.id, driver);
    } else {
      driver.full_name = input.full_name; driver.phone = input.phone; driver.active = input.active; driver.updated_at = when;
    }
    const event = { id:crypto.randomUUID(), driver_id:driver.id, action:'driver_saved', actor_id:input.actor_id,
      actor_role:input.actor_role, idempotency_key:input.idempotency_key,
      request:{ driver_id:input.driver_id || null, full_name:input.full_name, phone:input.phone, active:input.active },
      result:clone(driver), occurred_at:when };
    this.operationalEvents.set(event.id, event);
    return { duplicate:false, driver:clone(driver) };
  }

  async assignSystemDriver(input) {
    const prior = [...this.systemDriverAssignments.values()].find((row) => row.idempotency_key === input.idempotency_key);
    ...
    if (prior) {
      if (prior.order_id !== order.id || prior.driver_id !== input.driver_id || prior.leg !== input.leg
        || prior.assigned_by !== input.actor_id || prior.actor_role !== input.actor_role) {
        throw new InvalidTransitionError('Idempotency key conflicts with another driver assignment.');
      }
      return { duplicate:true, assignment:clone(prior) };
    }
    const driver = this.systemDrivers.get(input.driver_id);
    if (!driver?.active) throw new InvalidTransitionError('An active driver is required.');
    if (input.leg === 'pickup' && !['accepted', 'pickup_scheduled'].includes(order.order_status)) {
      throw new InvalidTransitionError('Pickup driver cannot be assigned from the current state.');
    }
    if (input.leg === 'delivery' && order.production_state !== 'ready') {
      throw new InvalidTransitionError('Delivery driver requires a ready order.');
    }
    const when = iso(input.occurred_at);
    for (const row of this.systemDriverAssignments.values()) {
      if (row.order_id === order.id && row.leg === input.leg && row.superseded_at == null) row.superseded_at = when;
    }
    const assignment = { id:crypto.randomUUID(), order_id:order.id, order_number:order.order_number,
      driver_id:driver.id, driver_name:driver.full_name, leg:input.leg, assigned_by:input.actor_id,
      actor_role:input.actor_role, idempotency_key:input.idempotency_key, assigned_at:when, superseded_at:null };
    this.systemDriverAssignments.set(assignment.id, assignment);
    const event = { id:crypto.randomUUID(), order_id:order.id, action:`assign_${input.leg}_driver`,
      actor_id:input.actor_id, actor_role:input.actor_role, idempotency_key:input.idempotency_key,
      previous_state:{}, new_state:{ driver_name:driver.full_name, leg:input.leg }, occurred_at:when };
    this.operationalEvents.set(event.id, event);
    return { duplicate:false, assignment:clone(assignment) };
  }

  async recordSystemManualPayment(input) {
    const order = [...this.orders.values()].find((row) => row.order_number === input.order_number);
    if (!order) throw new InvalidTransitionError('Order not found.');
    const prior = [...this.systemManualPayments.values()].find((row) => row.idempotency_key === input.idempotency_key);
    if (prior) {
      if (prior.order_id !== order.id || prior.method !== input.method || prior.amount !== input.amount
        || prior.tip_amount !== input.tip_amount || prior.paid_at !== input.paid_at
        || prior.reference !== input.reference || prior.source !== input.source
        || prior.note !== input.note || prior.recorded_by !== input.actor_id
        || prior.actor_role !== input.actor_role) {
        throw new InvalidTransitionError('Idempotency key conflicts with another payment.');
      }
      return { duplicate:true, payment:clone(prior) };
    }
    if (order.payment_status === 'paid') throw new InvalidTransitionError('Order is already paid.');
    if (!order.invoice_id || order.service_amount == null || !['invoice_created', 'failed'].includes(order.payment_status)) {
      throw new InvalidTransitionError('A current payable invoice is required.');
    }
    const expectedTotal = Math.round((Number(order.service_amount) + Number(input.tip_amount || 0)) * 100) / 100;
    if (expectedTotal !== Number(input.amount)) {
      throw new InvalidTransitionError('Payment total must equal service amount plus tip.');
    }
    if ([...this.payments.values()].some((row) => row.order_id === order.id)) {
      throw new InvalidTransitionError('A reconciled Stripe payment already exists.');
    }
    const payment = { id:crypto.randomUUID(), order_id:order.id, order_number:order.order_number,
      method:input.method, amount:input.amount, service_amount:Number(order.service_amount),
      tip_amount:Number(input.tip_amount || 0), total_amount:input.amount,
      invoice_id:order.current_invoice_id || order.invoice_id, reference:input.reference || null,
      source:input.source, currency:'USD', paid_at:input.paid_at,
      note:input.note || null, recorded_by:input.actor_id, actor_role:input.actor_role,
      idempotency_key:input.idempotency_key, created_at:iso(input.occurred_at) };
    this.systemManualPayments.set(payment.id, payment);
    order.payment_status = 'paid'; order.payment_id = `manual_${payment.id.replace(/-/g, '')}`;
    order.paid_at = payment.paid_at; order.currency = 'USD'; order.tip_amount = payment.tip_amount;
    order.updated_at = iso(input.occurred_at); order.version += 1;
    const event = { id:crypto.randomUUID(), order_id:order.id, action:'manual_payment_recorded',
      actor_id:input.actor_id, actor_role:input.actor_role, idempotency_key:input.idempotency_key,
      previous_state:{ payment_status:'invoice_created' }, new_state:{ payment_status:'paid', method:input.method,
        invoice_id:payment.invoice_id, source:payment.source, reference:payment.reference,
        service_amount:payment.service_amount, tip_amount:payment.tip_amount, total_amount:payment.total_amount },
      occurred_at:iso(input.occurred_at) };
    this.operationalEvents.set(event.id, event);
    return { duplicate:false, payment:clone(payment) };
  }

  async upsertSystemHotel(input) {
    const existingEvent = this.hotelEvents.get(input.idempotency_key);
    if (existingEvent) return clone(this.hotels.get(existingEvent.hotel_id));
    const aliases = [...new Set(input.aliases || [])];
    const keys = [input.canonical_name, ...aliases].map((value) => value.toLowerCase());
    const duplicate = [...this.hotels.values()].find((hotel) => hotel.id !== input.hotel_id
      && [hotel.canonical_name, ...(hotel.aliases || [])].some((value) => keys.includes(String(value).toLowerCase())));
    if (duplicate) throw new InvalidTransitionError('Hotel name or alias already exists.');
    const now = new Date().toISOString();
    const prior = input.hotel_id ? this.hotels.get(input.hotel_id) : null;
    if (input.hotel_id && !prior) throw new InvalidTransitionError('Hotel not found.');
    const hotel = { id:input.hotel_id || crypto.randomUUID(), unit_key:'orlando', canonical_name:input.canonical_name,
      address_line:input.address_line, region:input.region, aliases, handoff_notes:input.handoff_notes,
      active:input.active, created_by:prior?.created_by || input.actor.actor_id, updated_by:input.actor.actor_id,
      created_at:prior?.created_at || now, updated_at:now };
    this.hotels.set(hotel.id, hotel);
    this.hotelEvents.set(input.idempotency_key, { hotel_id:hotel.id, actor_id:input.actor.actor_id,
      action:prior ? 'hotel_updated' : 'hotel_created', occurred_at:now });
    return clone(hotel);
  }
  ...
  async getSystemPickupOrderByNumber(orderNumber) {
    ...
    return {
      ...
      service: {
        code: canonicalServiceCode(order.service_tier),
      ...
      delivery: {
        estimated_return: operational.estimated_return || operational.needed_by || null,
  ...
  operationalRow(order) {
    ...
    const activeAssignments = [...this.systemDriverAssignments.values()]
      .filter((row) => row.order_id === order.id && row.superseded_at == null);
    const pickupDriver = activeAssignments.find((row) => row.leg === 'pickup') || null;
    const deliveryDriver = activeAssignments.find((row) => row.leg === 'delivery') || null;
    const manualPayment = [...this.systemManualPayments.values()].find((row) => row.order_id === order.id) || null;
    return {
      ...
      invoice_id:order.invoice_id || null, service_amount:order.service_amount ?? null,
      tip_amount:order.tip_amount ?? null, paid_at:order.paid_at || null,
      ...
      pickup_driver:pickupDriver ? { driver_id:pickupDriver.driver_id, name:pickupDriver.driver_name,
        assigned_at:pickupDriver.assigned_at } : null,
      delivery_driver:deliveryDriver ? { driver_id:deliveryDriver.driver_id, name:deliveryDriver.driver_name,
        assigned_at:deliveryDriver.assigned_at } : null,
      manual_payment:manualPayment ? { method:manualPayment.method, amount:manualPayment.amount,
        service_amount:manualPayment.service_amount, tip_amount:manualPayment.tip_amount,
        total_amount:manualPayment.total_amount, reference:manualPayment.reference,
        source:manualPayment.source, invoice_id:manualPayment.invoice_id,
        currency:manualPayment.currency, paid_at:manualPayment.paid_at } : null,
      delivery_handoff:[...this.operationalEvents.values()].filter((event) => event.order_id === order.id && event.handoff_point)
  ...
  async getSystemOperationalSnapshot() {
    ...
    const waitingLeads = [...this.leads.values()].filter((lead) => ['new', 'qualifying', 'qualified'].includes(lead.status)
      ...
      .map((lead) => {
        ...
        return { lead_id:lead.id, customer_name:customer?.profile_name || 'Customer', whatsapp_last4:String(customer?.wa_id || '').slice(-4) || null,
  ...
  async getSystemActionableLeadById(leadId) {
    const lead = this.leads.get(leadId);
    if (!lead || !['new', 'qualifying', 'qualified'].includes(lead.status)
      || [...this.orders.values()].some((order) => order.lead_id === leadId)) return null;
    const customer = this.customers.get(lead.customer_id);
    return { ...clone(lead), lead_id:lead.id,
      customer_name:customer?.profile_name || lead.operational_data?.name || 'Customer',
      whatsapp_number:customer?.wa_id || lead.operational_data?.whatsapp_number || null };
  }

  async resolveExistingLeadOrderRetry(input) {
    const existing = this.manualRequests?.get(input.submission_id);
    if (!existing) return null;
    if (existing.fingerprint !== input.request_fingerprint || existing.lead_id !== input.lead_id) {
      throw new InvalidTransitionError('Idempotency key conflicts with another order.');
    }
    const order = this.orders.get(existing.order_id);
    return { duplicate:true, customer_reused:true, order_number:order?.order_number || existing.order_number };
  }
  ...
  async resolveSystemMessageCreateRetry(input) {
    const order = [...this.orders.values()].find((row) => row.order_number === input.order_number);
    if (!order) throw new InvalidTransitionError('Order not found.');
    ...
    if (prior.action !== 'draft_created' || prior.order_id !== order.id
      || draft?.template_key !== input.template_key) {
      throw new InvalidTransitionError('Idempotency key conflicts with another message draft.');
    }
  ...
  async getSystemPaymentLink(orderId) {
    const row = [...this.systemPaymentLinks.values()].find((item) => item.order_id === orderId
      && ['creating', 'active'].includes(item.status));
    return clone(row || null);
  }

  async getSystemPaymentLinkOrder(orderNumber) {
    return clone([...this.orders.values()].find((row) => row.order_number === orderNumber) || null);
  }

  async reserveSystemPaymentLink(input) {
    const order = this.orders.get(input.order_id);
    if (!order || order.lead_id !== input.lead_id || order.invoice_id !== input.invoice_id
      || (order.current_invoice_id && order.current_invoice_id !== input.invoice_id)) {
      throw new InvalidTransitionError('Current payable invoice is inconsistent.');
    }
    if (!['invoice_created', 'failed', 'void'].includes(order.payment_status)
      || Number(order.service_amount) !== Number(input.service_amount)) {
      throw new InvalidTransitionError('Current payable invoice is required.');
    }
    const current = [...this.systemPaymentLinks.values()].find((row) => row.order_id === order.id
      && row.invoice_id === input.invoice_id && ['creating', 'active'].includes(row.status));
    if (current) {
      if (current.request_fingerprint !== input.request_fingerprint) {
        throw new InvalidTransitionError('A different Payment Link is already current for this invoice.');
      }
      return { duplicate:true, payment_link:clone(current) };
    }
    const failed = [...this.systemPaymentLinks.values()].find((row) => row.order_id === order.id
      && row.invoice_id === input.invoice_id && row.status === 'failed'
      && row.request_fingerprint === input.request_fingerprint);
    if (failed) {
      failed.status = 'creating'; failed.error_code = null; failed.updated_at = iso(input.occurred_at);
      return { duplicate:true, payment_link:clone(failed) };
    }
    const when = iso(input.occurred_at);
    const row = {
      id:crypto.randomUUID(), payment_link_attempt_id:null, order_id:order.id,
      order_number:order.order_number || null, invoice_id:input.invoice_id,
      invoice_version:Number(this.systemInvoices.get(input.invoice_id)?.version || 1),
      service_amount:Number(input.service_amount), tip_amount:Number(input.tip_amount),
      total_amount:Number(input.total_amount), currency:'USD', status:'creating',
      request_fingerprint:input.request_fingerprint, idempotency_key:input.idempotency_key,
      actor_id:input.actor_id, actor_role:input.actor_role,
      stripe_service_price_id:null, stripe_tip_price_id:null,
      stripe_payment_link_id:null, stripe_url:null, error_code:null,
      created_at:when, updated_at:when
    };
    row.payment_link_attempt_id = row.id;
    this.systemPaymentLinks.set(row.id, row);
    return { duplicate:false, payment_link:clone(row) };
  }

  async activateSystemPaymentLink(input) {
    const row = this.systemPaymentLinks.get(input.payment_link_attempt_id);
    if (!row || row.status !== 'creating') throw new InvalidTransitionError('Payment Link reservation is not current.');
    row.status = 'active';
    row.stripe_service_price_id = input.stripe_service_price_id;
    row.stripe_tip_price_id = input.stripe_tip_price_id || null;
    row.stripe_payment_link_id = input.stripe_payment_link_id;
    row.stripe_url = input.stripe_url;
    row.updated_at = iso(input.occurred_at);
    const invoice = this.systemInvoices.get(row.invoice_id);
    if (invoice) invoice.payment_link_id = row.stripe_payment_link_id;
    const order = this.orders.get(row.order_id);
    if (order) order.payment_link_id = row.stripe_payment_link_id;
    return { payment_link:clone(row) };
  }

  async failSystemPaymentLink(input) {
    const row = this.systemPaymentLinks.get(input.payment_link_attempt_id);
    if (!row || row.status !== 'creating') return { ignored:true };
    row.status = 'failed'; row.error_code = input.error_code; row.updated_at = iso(input.occurred_at);
    return { ignored:false };
  }

  async getSystemPaymentLinkByStripeId(paymentLinkId) {
    return clone([...this.systemPaymentLinks.values()].find((row) => row.stripe_payment_link_id === paymentLinkId) || null);
  }

  async resolveSystemInvoiceActionRetry(input) {
    const order = [...this.orders.values()].find((row) => row.order_number === input.order_number);
    if (!order) throw new InvalidTransitionError('Order not found.');
    const prior = [...this.systemInvoiceEvents.values()].find((row) => row.idempotency_key === input.idempotency_key);
    if (!prior) return null;
    if (prior.action !== input.action || prior.order_id !== order.id
  ...
  async reviewSystemInvoice(input) {
    ...
    if (!['awaiting_processing', 'processing', 'ready'].includes(order.production_state)
      || order.order_status === 'cancelled') {
      throw new InvalidTransitionError('Order must be weighed before invoice review.');
    }
  ...
  async transitionSystemOperationalOrder(input) {
    ...
    if (input.action === 'schedule_pickup') {
      ...
      if (![...this.systemDriverAssignments.values()].some((row) => row.order_id === order.id
        && row.leg === 'pickup' && !row.superseded_at)) {
        throw new InvalidTransitionError('Pickup driver assignment required.');
      }
      ...
      if (![...this.systemDriverAssignments.values()].some((row) => row.order_id === order.id
        && row.leg === 'delivery' && !row.superseded_at)) {
        throw new InvalidTransitionError('Delivery driver assignment required.');
      }
      ...
      if (!['bell_desk', 'front_desk', 'concierge'].includes(input.handoff_point)) {
        throw new InvalidTransitionError('A hotel handoff point is required.');
      }
      ...
      if (order.custody_state === 'with_driver_delivery'
        && !['guest', 'other'].includes(input.handoff_point)) {
        throw new InvalidTransitionError('A direct handoff point is required.');
      }
      if (input.handoff_point === 'other' && !input.handoff_note) {
    ...
    const event = { id:crypto.randomUUID(), order_id:order.id, order_number:order.order_number,
      ...
      new_state:{ custody_state:order.custody_state || null, production_state:order.production_state || null,
        order_status:order.order_status, promised_by:order.promised_by || null }, reason:input.reason || null,
      handoff_point:input.handoff_point || null, handoff_note:input.handoff_note || null, occurred_at:when };
    this.operationalEvents.set(event.id, event);
    return { duplicate:false };
  }

  async initializeSystemLegacyOrder(input) {
    const existing = [...this.operationalEvents.values()].find((event) => event.idempotency_key === input.idempotency_key);
    const order = [...this.orders.values()].find((row) => row.order_number === input.order_number);
    if (!order) throw new InvalidTransitionError('Order not found.');
    const lead = this.leads.get(order.lead_id);
    ...
    if (isQaOrder(order, lead, customer)) throw new InvalidTransitionError('QA orders are read-only.');
    if (existing) {
      if (existing.action !== 'initialize_legacy_order' || existing.order_id !== order.id
        || existing.actor_id !== input.actor_id || existing.actor_role !== input.actor_role
        || existing.reason !== input.reason) {
        throw new InvalidTransitionError('Idempotency key conflicts with another legacy initialization.');
      }
      return { duplicate:true };
    }
    if (input.actor_role !== 'owner') throw new InvalidTransitionError('Legacy order initialization requires Owner authorization.');
    if (!input.reason) throw new InvalidTransitionError('A reason is required to initialize a legacy order.');
    if (order.order_status !== 'accepted' || order.custody_state != null || order.production_state != null) {
      throw new InvalidTransitionError('Only an accepted uninitialized legacy order can be initialized.');
    }
    const when = iso(input.occurred_at);
    const before = { custody_state:null, production_state:null, order_status:order.order_status,
      promised_by:order.promised_by || null };
    order.custody_state = 'with_customer';
    order.production_state = 'awaiting_intake';
    order.operational_waiting_since = when;
    order.updated_at = new Date().toISOString();
    order.version = (order.version || 0) + 1;
    const event = { id:crypto.randomUUID(), order_id:order.id, order_number:order.order_number,
      action:'initialize_legacy_order', actor_id:input.actor_id, actor_role:input.actor_role,
      idempotency_key:input.idempotency_key, previous_state:before,
      new_state:{ custody_state:order.custody_state, production_state:order.production_state,
        order_status:order.order_status, promised_by:order.promised_by || null },
      reason:input.reason, occurred_at:when };
    this.operationalEvents.set(event.id, event);
    return { duplicate:false };
  }
  ...
  async runW1cASmokeProbe(input) {
    ...
    isolated.customers.set(customerId, {
      id:customerId, unit_key:'orlando', wa_id:`system-weight-smoke-${input.request_id}`,
      profile_name:'System Weight Smoke Fixture'
    });
    isolated.leads.set(leadId, {
      id:leadId, customer_id:customerId, status:'order_accepted', language:'en',
      accommodation_type:'hotel', operational_data:{ property:'System Weight Smoke Fixture' }
    });
    isolated.orders.set(orderId, {
      id:orderId, lead_id:leadId, customer_id:customerId, order_number:'MCO 990000000001',
      order_status:'picked_up', payment_status:'pending', service_tier:'normal', pricing_model:'per_lb',
      custody_state:'at_laundry', production_state:'awaiting_weight',
      operational_waiting_since:iso(input.occurred_at), version:1
    });
    isolated.orderItems.set(orderId, [{
      id:itemId, order_id:orderId, catalog_code:'wash_fold_guest', service_type:'wash_fold_guest',
      label:'System Weight Smoke Item', unit:'lb', estimated_lbs:5, unit_price:3.25,
      minimum_amount:50, requires_manual_review:false, actual_lbs:null, weighed_at:null,
      subtotal:null, weight_version:0
    }]);
    const weight = {
      order_number:'MCO 990000000001', order_item_id:itemId, actual_lbs:5,
      expected_weight_version:0, actor_id:input.actor_id, actor_role:input.actor_role,
      idempotency_key:`w1c-a-smoke:${input.request_id}`, reason:'Transactional smoke probe',
      occurred_at:iso(input.occurred_at)
    };
    const first = await isolated.recordSystemItemWeight(weight);
    const retry = await isolated.recordSystemItemWeight(weight);
    const order = isolated.orders.get(orderId);
    const weightEventCount = isolated.itemWeightEvents.size;
    const lifecycleEventCount = [...isolated.events.values()]
      .filter((event) => event.order_id === orderId && event.event_name === 'order_weighed').length;
    return {
      passed:first.duplicate === false && retry.duplicate === true
        && weightEventCount === 1 && lifecycleEventCount === 1
        && order.order_status === 'weighed' && order.production_state === 'awaiting_processing'
        && Number(order.actual_lbs) === 5,
      first_duplicate:Boolean(first.duplicate), retry_duplicate:Boolean(retry.duplicate),
      weight_event_count:weightEventCount, lifecycle_event_count:lifecycleEventCount,
      final_order_status:order.order_status, final_production_state:order.production_state,
      actual_lbs:Number(order.actual_lbs), residue_count:0
    };
  }

  async runW3dSmokeProbe() {
    throw new OperationalStorageUnavailableError(
      'W3-D transactional smoke is available only through the protected Production database authority.'
    );
  }
  ...
  async getSystemOwnerFinance(input) {
    ...
    for (const order of this.orders.values()) {
      const lead = this.leads.get(order.lead_id);
      const customer = this.customers.get(order.customer_id);
      if (!order.order_number || order.order_status === 'cancelled' || isQaOrder(order, lead, customer)) continue;
      const payment = [...this.payments.values()]
        .filter((row) => row.order_id === order.id
          && ['paid', 'partially_refunded', 'refunded'].includes(row.status)
          && inPeriod(row.paid_at))
        .sort((left, right) => String(right.paid_at || '').localeCompare(String(left.paid_at || ''))
          || String(right.id || '').localeCompare(String(left.id || '')))[0];
      const serviceAmount = Number(order.service_amount);
      const revenue = payment && ['paid', 'partially_refunded', 'refunded'].includes(order.payment_status)
        && Number.isFinite(serviceAmount)
        ? Math.max(serviceAmount - (Number(payment.refund_total) || 0), 0) : null;
      if (payment && revenue !== null) {
        const historicalTip = lead?.operational_data
          && Object.hasOwn(lead.operational_data, 'historical_tip_amount')
          ? Number(lead.operational_data.historical_tip_amount) : null;
        const explicitTip = Number.isFinite(historicalTip) ? historicalTip
          : order.tip_amount == null ? null : Number(order.tip_amount);
        const hotel = order.hotel_id ? this.hotels.get(order.hotel_id) : null;
        paid.push({ order, payment, revenue, tip:Number.isFinite(explicitTip) ? explicitTip : null,
          hotel:hotel?.canonical_name || 'Unmapped / other',
          acquisition:acquisitionSource(order) || 'Unattributed' });
      } else if (inPeriod(order.accepted_at)
        && !['paid', 'partially_refunded', 'refunded', 'void'].includes(order.payment_status)) {
        pending.push(order);
      }
    }
    const total = (rows, pick) => money(rows.reduce((sum, row) => sum + Number(pick(row) || 0), 0));
    const confirmedService = total(paid, (row) => row.revenue);
    const knownTips = paid.filter((row) => row.tip !== null);
    const knownPending = pending.filter((order) => order.service_amount != null && Number.isFinite(Number(order.service_amount)));
    const breakdown = (pick) => {
      const groups = new Map();
      for (const row of paid) {
        const bucket = pick(row);
        const current = groups.get(bucket) || { bucket, paid_order_count:0, confirmed_service_revenue:0 };
        current.paid_order_count += 1;
        current.confirmed_service_revenue = money(current.confirmed_service_revenue + row.revenue);
        groups.set(bucket, current);
      }
      return [...groups.values()].sort((left, right) => right.confirmed_service_revenue - left.confirmed_service_revenue
        || left.bucket.localeCompare(right.bucket));
    };
    return {
      period:{ start_date:startDate, end_date:endDate, timezone:'America/New_York', basis:'authoritative_paid_at' },
      summary:{ currency:'USD', paid_order_count:paid.length,
        customer_count:new Set(paid.map((row) => row.order.customer_id).filter(Boolean)).size,
        confirmed_service_revenue:confirmedService,
        gross_received:total(paid, (row) => Math.max(Number(row.payment.amount) - Number(row.payment.refund_total || 0), 0)),
        confirmed_tips:knownTips.length ? total(knownTips, (row) => row.tip) : null,
        average_service_ticket:paid.length ? money(confirmedService / paid.length) : null,
        new_customer_orders:paid.filter((row) => !row.order.is_repeat_customer).length,
        repeat_customer_orders:paid.filter((row) => row.order.is_repeat_customer).length,
        normal_paid_orders:paid.filter((row) => row.order.service_tier === 'normal').length,
        express_paid_orders:paid.filter((row) => row.order.service_tier === 'express').length,
        pending_payment_count:pending.length,
        pending_payment_value:knownPending.length ? total(knownPending, (order) => order.service_amount) : null },
      availability:{ status:'current', service_revenue:'current', gross_received:'current',
        tips:paid.length === 0 ? 'no_data' : knownTips.length === paid.length ? 'current'
          : knownTips.length === 0 ? 'unavailable' : 'partial',
        pending_payment_value:pending.length === 0 ? 'no_data' : knownPending.length === pending.length ? 'current'
          : knownPending.length === 0 ? 'unavailable' : 'partial',
        processing_fees:'unavailable', net_payout:'unavailable' },
      breakdowns:{ service:breakdown((row) => row.order.service_tier || 'unknown'),
        hotel:breakdown((row) => row.hotel), acquisition:breakdown((row) => row.acquisition) },
      sources:['A7 Orlando orders', 'reconciled payments/refunds',
        'protected explicit tip facts', 'frozen attribution snapshots'],
      freshness:{ generated_at:new Date().toISOString() }
    };
  }

  async resolveKnownCustomerOrderRetry(input) {
    this.manualRequests ||= new Map();
    const existing = this.manualRequests.get(input.submission_id);
    if (!existing) return null;
    const order = this.orders.get(existing.order_id);
    if (existing.fingerprint !== input.request_fingerprint
      || order?.customer_id !== input.customer_id) {
      throw new InvalidTransitionError('Idempotency key conflicts with another known-customer order.');
    }
    return {
      duplicate:true,
  ...
  async recordPayment(input) {
    const existingEvent = this.stripeEvents.get(input.stripe_event_id);
    if (existingEvent) {
      const payment = this.payments.get(input.transaction_id) || null;
      const serviceAmount = Number(input.service_amount == null ? payment?.service_amount ?? input.amount : input.service_amount);
      const tipAmount = Number(input.tip_amount || 0);
      const totalAmount = Number(input.total_amount == null ? input.amount : input.total_amount);
      if (!payment || existingEvent.event_type !== input.event_type
        || existingEvent.object_id !== (input.checkout_session_id || null)
        || existingEvent.order_id !== input.order_id
        || existingEvent.transaction_id !== input.transaction_id
        || existingEvent.payment_link_id !== (input.payment_link_id || null)
        || existingEvent.service_amount !== serviceAmount
        || existingEvent.tip_amount !== tipAmount
        || existingEvent.total_amount !== totalAmount
        || existingEvent.currency !== input.currency
        || payment.order_id !== input.order_id
        || payment.checkout_session_id !== (input.checkout_session_id || null)
        || payment.payment_link_id !== (input.payment_link_id || null)
        || Number(payment.service_amount) !== serviceAmount
        || Number(payment.tip_amount) !== tipAmount
        || Number(payment.total_amount) !== totalAmount
        || payment.currency !== input.currency) {
        throw new InvalidTransitionError('Stripe event idempotency conflict.');
      }
      return { duplicate: true, payment: clone(payment) };
    }
    const order = this.orders.get(input.order_id);
    if (!order || !order.invoice_id) throw new InvalidTransitionError('Invoiced order required.');
    const link = input.payment_link_id ? await this.getSystemPaymentLinkByStripeId(input.payment_link_id) : null;
    const serviceAmount = input.service_amount == null ? Number(order.service_amount) : Number(input.service_amount);
    const tipAmount = Number(input.tip_amount || 0);
    const totalAmount = input.total_amount == null ? Number(input.amount) : Number(input.total_amount);
    if (Number(input.amount) !== totalAmount || serviceAmount !== Number(order.service_amount)
      || totalAmount !== Math.round((serviceAmount + tipAmount) * 100) / 100 || input.currency !== 'USD') {
      throw new InvalidTransitionError('Payment total does not match service amount plus tip.');
    }
    if (input.tip_amount != null && (!link || link.order_id !== order.id || link.status !== 'active'
      || Number(link.service_amount) !== serviceAmount || Number(link.tip_amount) !== tipAmount
      || Number(link.total_amount) !== totalAmount)) {
      throw new InvalidTransitionError('Payment does not match its governed Payment Link.');
    }
    ...
    if (!payment) {
      payment = {
        ...
        payment_link_id: input.payment_link_id || null, amount:totalAmount,
        invoice_id:order.current_invoice_id || order.invoice_id, service_amount:serviceAmount,
        tip_amount:tipAmount, total_amount:totalAmount, currency: 'USD',
      ...
      if (link) {
        const governed = this.systemPaymentLinks.get(link.id);
        governed.status = 'completed'; governed.completed_at = payment.paid_at;
        governed.updated_at = new Date().toISOString();
      }
    }
...
class SupabaseOperationalStore {
  ...
  async listSystemHotels(input = {}) {
    return this.rpc('a7_orlando_list_hotels', {
      p_query:input.query || '',
      p_include_inactive:Boolean(input.include_inactive),
      p_limit:Math.max(1, Math.min(Number(input.limit) || 100, 200))
    });
  }

  async listSystemDrivers(input = {}) {
    return this.rpc('a7_orlando_list_drivers', { p_include_inactive:Boolean(input.include_inactive) });
  }

  async listRoutes(input = {}) {
    return this.rpc('a7_orlando_list_routes', { p_route_date:input.route_date || null });
  }

  async getRoute(routeId) {
    return this.rpc('a7_orlando_route_payload', { p_route_id:routeId });
  }

  async listEligibleStops(input = {}) {
    return this.rpc('a7_orlando_route_eligible_stops', { p_route_id:input.route_id });
  }

  async routeCommand(command, input, payload) {
    return this.rpc('a7_orlando_route_command', {
      p_command:command, p_route_id:input.route_id || null, p_payload:payload,
      p_actor_id:input.actor_id, p_actor_role:input.actor_role,
      p_idempotency_key:input.idempotency_key, p_occurred_at:iso(input.occurred_at)
    });
  }

  async createRoute(input) {
    return this.routeCommand('create', input, { route_date:input.route_date, driver_id:input.driver_id });
  }

  async addRouteStop(input) {
    return this.routeCommand('add_stop', input, { order_number:input.order_number, stop_type:input.stop_type, eta_at:input.eta_at });
  }

  async removeRouteStop(input) {
    return this.routeCommand('remove_stop', input, { stop_id:input.stop_id });
  }

  async reorderRouteStops(input) {
    return this.routeCommand('reorder', input, { stop_ids:input.stop_ids, version:input.version });
  }

  async setRouteStopEta(input) {
    return this.routeCommand('set_eta', input, { stop_id:input.stop_id, eta_at:input.eta_at, version:input.version });
  }

  async startRoute(input) { return this.routeCommand('start', input, { version:input.version }); }
  async executeRouteStop(input) {
    return this.routeCommand('execute_stop', input, { stop_id:input.stop_id, action:input.action,
      handoff_point:input.handoff_point, handoff_note:input.handoff_note });
  }
  async recordRouteStopException(input) {
    return this.routeCommand('exception', input, { stop_id:input.stop_id, reason:input.reason, note:input.note });
  }
  async completeRoute(input) { return this.routeCommand('complete', input, { version:input.version }); }
  async cancelRoute(input) { return this.routeCommand('cancel', input, { version:input.version }); }

  async upsertSystemDriver(input) {
    return this.rpc('a7_orlando_upsert_driver', {
      p_driver_id:input.driver_id || null, p_full_name:input.full_name, p_phone:input.phone,
      p_active:input.active, p_actor_id:input.actor_id, p_actor_role:input.actor_role,
      p_idempotency_key:input.idempotency_key, p_occurred_at:iso(input.occurred_at)
    });
  }

  async assignSystemDriver(input) {
    return this.rpc('a7_orlando_assign_driver', {
      p_order_number:input.order_number, p_driver_id:input.driver_id, p_leg:input.leg,
      p_actor_id:input.actor_id, p_actor_role:input.actor_role,
      p_idempotency_key:input.idempotency_key, p_occurred_at:iso(input.occurred_at)
    });
  }
  ...
  async recordSystemManualPayment(input) {
    return this.rpc('a7_orlando_record_manual_payment_v2', {
      p_order_number:input.order_number, p_method:input.method, p_amount:input.amount,
      p_tip_amount:input.tip_amount, p_paid_at:input.paid_at, p_reference:input.reference || null,
      p_source:input.source, p_note:input.note || null, p_actor_id:input.actor_id,
      p_actor_role:input.actor_role, p_idempotency_key:input.idempotency_key,
      p_occurred_at:iso(input.occurred_at)
    });
  }

  async upsertSystemHotel(input) {
    return this.rpc('a7_orlando_upsert_hotel', {
      ...
      p_handoff_notes:input.handoff_notes,
  ...
  async createManualOrder(input) {
    return this.rpc('a7_orlando_create_manual_order_v3', {
      ...
      p_promised_by: input.promised_by,
      p_items: input.items,
      p_occurred_at: new Date().toISOString()
    });
  }
  ...
  async createKnownCustomerOrder(input) {
    return this.rpc('a7_orlando_create_known_customer_order_v2', {
      p_submission_id:input.submission_id,
      p_request_fingerprint:input.request_fingerprint,
      p_actor_id:input.actor.actor_id,
      p_actor_role:input.actor.role,
  ...
  async resolveKnownCustomerOrderRetry(input) {
    return this.rpc('a7_orlando_resolve_known_customer_order_retry', {
      p_submission_id:input.submission_id,
      p_request_fingerprint:input.request_fingerprint,
      p_customer_id:input.customer_id
    });
  ...
  async createExistingLeadOrder(input) {
    return this.rpc('a7_orlando_accept_existing_lead_order_v2', {
      p_submission_id:input.submission_id,
      p_request_fingerprint:input.request_fingerprint,
      p_actor_id:input.actor.actor_id,
      p_actor_role:input.actor.role,
  ...
  async resolveExistingLeadOrderRetry(input) {
    return this.rpc('a7_orlando_resolve_existing_lead_order_retry', {
      p_submission_id:input.submission_id,
      p_request_fingerprint:input.request_fingerprint,
      p_lead_id:input.lead_id
    });
  ...
  async getSystemOwnerFinance(input) {
    return this.rpc('a7_orlando_owner_finance', {
  ...
  async getSystemOperationalSnapshot() {
    const [snapshot, waitingLeads] = await Promise.all([
      this.rpc('a7_orlando_operational_cycle_snapshot', {}),
      this.rpc('a7_orlando_actionable_leads', {})
    ]);
    return { ...snapshot,
      waiting_confirmation:Array.isArray(waitingLeads) ? waitingLeads.length : snapshot?.waiting_confirmation,
      waiting_leads:Array.isArray(waitingLeads) ? waitingLeads : [] };
  }

  async getSystemActionableLeadById(leadId) {
    return this.rpc('a7_orlando_actionable_lead', { p_lead_id:leadId });
  }
  ...
  async getSystemW1cOrderByNumber(orderNumber) {
    return this.rpc('a7_orlando_operational_cycle_order', { p_order_number:orderNumber });
  }
  ...
  async getSystemPaymentLink(orderId) {
    return this.rpc('a7_orlando_payment_link_current', { p_order_id:orderId });
  }

  async getSystemPaymentLinkOrder(orderNumber) {
    return this.rpc('a7_orlando_payment_link_order', { p_order_number:orderNumber });
  }

  async reserveSystemPaymentLink(input) {
    return this.rpc('a7_orlando_payment_link_reserve', {
      p_order_id:input.order_id, p_lead_id:input.lead_id, p_invoice_id:input.invoice_id,
      p_service_amount:input.service_amount, p_tip_amount:input.tip_amount,
      p_total_amount:input.total_amount, p_request_fingerprint:input.request_fingerprint,
      p_actor_id:input.actor_id, p_actor_role:input.actor_role,
      p_idempotency_key:input.idempotency_key, p_occurred_at:iso(input.occurred_at)
    });
  }
  ...
  async activateSystemPaymentLink(input) {
    return this.rpc('a7_orlando_payment_link_activate', {
      p_attempt_id:input.payment_link_attempt_id,
      p_service_price_id:input.stripe_service_price_id,
      p_tip_price_id:input.stripe_tip_price_id || null,
      p_payment_link_id:input.stripe_payment_link_id, p_stripe_url:input.stripe_url,
      p_occurred_at:iso(input.occurred_at)
    });
  }

  async failSystemPaymentLink(input) {
    return this.rpc('a7_orlando_payment_link_fail', {
      p_attempt_id:input.payment_link_attempt_id, p_error_code:input.error_code,
      p_occurred_at:iso(input.occurred_at)
    });
  }

  async getSystemPaymentLinkByStripeId(paymentLinkId) {
    return this.rpc('a7_orlando_payment_link_by_stripe_id', { p_payment_link_id:paymentLinkId });
  }

  async resolveSystemInvoiceActionRetry(input) {
    return this.rpc('a7_orlando_w1c_b1_resolve_action_retry', {
  ...
  async reviewSystemInvoice(input) {
    return this.rpc('a7_orlando_w1c_b1_review_invoice_v2', {
      p_order_number:input.order_number,
      p_expected_invoice_version:input.expected_invoice_version,
  ...
  async resolveSystemMessageCreateRetry(input) {
    return this.rpc('a7_orlando_w2_a_resolve_create_retry', {
      p_order_number:input.order_number,
  ...
  async transitionSystemOperationalOrder(input) {
    if (input.actor_role === 'operator') {
      return this.rpc('a7_orlando_w1b_operator_mark_ready', {
        p_order_number:input.order_number,
        p_actor_id:input.actor_id,
        p_idempotency_key:input.idempotency_key,
        p_occurred_at:iso(input.occurred_at)
    ...
    return this.rpc('a7_orlando_operational_cycle_transition_v2', {
      ...
      p_promised_by:input.promised_by || null, p_handoff_point:input.handoff_point || null,
      p_handoff_note:input.handoff_note || null, p_occurred_at:iso(input.occurred_at)
    });
  }

  async initializeSystemLegacyOrder(input) {
    return this.rpc('a7_orlando_initialize_legacy_order', {
      p_order_number:input.order_number,
      p_actor_id:input.actor_id,
      p_actor_role:input.actor_role,
      p_idempotency_key:input.idempotency_key,
  ...
  async runW1cASmokeProbe(input) {
    return this.rpc('a7_orlando_w1c_a_transactional_smoke', {
      p_actor_id:input.actor_id,
      p_actor_role:input.actor_role,
      p_request_id:input.request_id
    });
  ...
  async runW3dSmokeProbe(input) {
    return this.rpc('a7_orlando_w3d_transactional_smoke', {
      p_actor_id:input.actor_id,
      p_actor_role:input.actor_role,
      p_request_id:input.request_id
    });
  ...
  async recordPayment(input) {
    if (input.service_amount != null || input.tip_amount != null || input.total_amount != null) {
      return this.rpc('a7_orlando_record_payment_v2', {
        p_stripe_event_id:input.stripe_event_id, p_event_type:input.event_type,
        p_order_id:input.order_id, p_transaction_id:input.transaction_id,
        p_checkout_session_id:input.checkout_session_id || null,
        p_payment_link_id:input.payment_link_id || null,
        p_service_amount:input.service_amount, p_tip_amount:input.tip_amount,
        p_total_amount:input.total_amount, p_currency:input.currency,
        p_paid_at:iso(input.paid_at)
      });
    }
  ...
  async recordRefund(input) {
    return this.rpc('a7_orlando_record_refund_v2', {
...
class UnavailableOperationalStore {
  ...
  async listSystemHotels() { return this.fail(); }
  async upsertSystemHotel() { return this.fail(); }
  async listSystemDrivers() { return this.fail(); }
  async listRoutes() { return this.fail(); }
  async getRoute() { return this.fail(); }
  async listEligibleStops() { return this.fail(); }
  async createRoute() { return this.fail(); }
  async addRouteStop() { return this.fail(); }
  async removeRouteStop() { return this.fail(); }
  async reorderRouteStops() { return this.fail(); }
  async setRouteStopEta() { return this.fail(); }
  async startRoute() { return this.fail(); }
  async executeRouteStop() { return this.fail(); }
  async recordRouteStopException() { return this.fail(); }
  async completeRoute() { return this.fail(); }
  async cancelRoute() { return this.fail(); }
  async upsertSystemDriver() { return this.fail(); }
  async assignSystemDriver() { return this.fail(); }
  async recordSystemManualPayment() { return this.fail(); }
  ...
  async createExistingLeadOrder() { return this.fail(); }
  async resolveExistingLeadOrderRetry() { return this.fail(); }
  async resolveKnownCustomerOrderRetry() { return this.fail(); }
  ...
  async getSystemOwnerFinance() { return this.fail(); }
  async getSystemOperationalSnapshot() { return this.fail(); }
  async getSystemActionableLeadById() { return this.fail(); }
  ...
  async getSystemPaymentLink() { return this.fail(); }
  async getSystemPaymentLinkOrder() { return this.fail(); }
  async reserveSystemPaymentLink() { return this.fail(); }
  async activateSystemPaymentLink() { return this.fail(); }
  async failSystemPaymentLink() { return this.fail(); }
  async getSystemPaymentLinkByStripeId() { return this.fail(); }
  async resolveSystemInvoiceActionRetry() { return this.fail(); }
  ...
  async resolveSystemMessageCreateRetry() { return this.fail(); }
  ...
  async initializeSystemLegacyOrder() { return this.fail(); }
  async runW1bSmokeProbe() { return this.fail(); }
  async runW1cASmokeProbe() { return this.fail(); }
  async runW3dSmokeProbe() { return this.fail(); }
