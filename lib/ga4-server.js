...
async function sendGa4Event(input, options = {}) {
  ...
  const configuredTimeout = Number(options.ga4TimeoutMs);
  const timeoutMs = Number.isFinite(configuredTimeout)
    ? Math.max(100, Math.min(configuredTimeout, 10_000))
  ...
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        controller.abort();
        const error = new Error('GA4 request timed out.');
        error.code = 'GA4_TIMEOUT';
        reject(error);
      }, timeoutMs);
    });
    response = await Promise.race([fetchPromise, timeoutPromise]);
  } catch (error) {
    if (error?.code === 'GA4_TIMEOUT' || error?.name === 'AbortError') {
      return { sent: false, status: 'failed', reason: 'request_timeout' };
    }
...
module.exports = {
  ALLOWED_EVENTS, MAX_BACKDATE_MS, MAX_FUTURE_SKEW_MS, DEFAULT_REQUEST_TIMEOUT_MS,
