...
function validate(input = {}) {
  ...
  const items = resolveItems(input.items, tier);
...
async function createInMemory(input, actor, store, options, knownCustomerId = null) {
  ...
  try {
    ...
    const lead = await lifecycle.createLead({
      ...
      operational_data: {
        property: input.property, property_address: input.property_address, room: input.room,
...
function systemOrderService(options = {}) {
  ...
  return {
    ...
    async createManualOrder(raw, actor) {
      if (!actor || !['owner', 'operator'].includes(actor.role)) throw new InvalidTransitionError('Operator authorization is required.');
      ...
      const input = validate({ ...raw, analytics_context: null });
    ...
    async createKnownCustomerOrder(raw, actor) {
      ...
      if (!customer || Number(customer.order_count) < 1) {
      ...
      const result = await store.createKnownCustomerOrder({
        ...
        request_fingerprint:fingerprint(protectedInput),
        actor
      });
      if (!result.duplicate) {
        await deliverOutbox(store, `order_accepted:${input.submission_id}`, options).catch(() => null);
    ...
    async getByOrderNumber(orderNumber) {
      ...
      const row = await store.getSystemOrderByNumber(value);
      if (!row) return null;
      return row;
    },
    async getPickupOrderByNumber(orderNumber) {
      const value = normalizeOrderNumber(orderNumber);
      if (!value) throw new InvalidTransitionError('Order number is invalid. Use 1002 or MCO 1002.');
      return store.getSystemPickupOrderByNumber(value);
    }
  };
}

module.exports = { systemOrderService, validate, fingerprint, safeResult, normalizeOrderNumber, ORDER_NUMBER };
