...
const QUEUES = new Set([
  ...
  'processing', 'ready', 'charge', 'awaiting_payment', 'deliveries', 'express', 'express_attention', 'late'
]);
...
function nextActionFor(order) {
  ...
  if (!CUSTODY_STATES.has(order.custody_state) || !PRODUCTION_STATES.has(order.production_state)) {
    return { code: 'review_state', label: 'REVISAR ESTADO OPERACIONAL', enabled: false };
  }
  ...
  if (order.production_state === 'ready' && ['invoice_created', 'failed'].includes(order.payment_status)) {
    return { code: 'await_payment', label: 'AGUARDAR PAGAMENTO', enabled: false };
  }
  ...
  if (order.production_state === 'ready' && order.custody_state === 'with_driver_delivery') {
    return order.accommodation_type === 'hotel'
      ? { code: 'leave_bell_desk', label: 'DEIXAR NO BELL DESK', enabled: true }
      : { code: 'complete_delivery', label: 'CONCLUIR ENTREGA', enabled: true };
  }
...
function isInQueue(order, queue, context) {
  ...
  if (queue === 'express_attention') return order.service_tier === 'express'
    && ['attention', 'risk', 'late'].includes(order.sla.status);
  if (queue === 'late') return order.sla.status === 'late' || order.standard_overdue;
  return false;
}

function priority(order) {
  const rank = order.sla.status === 'late' ? 0 : order.sla.status === 'risk' ? 1
    : order.standard_overdue ? 2 : 3;
...
function decorateSnapshot(raw, now) {
  ...
  return {
    ...
    counters: {
      ...
      express_attention: real.filter((order) => ['attention', 'risk', 'late'].includes(order.sla.status)).length
    },
...
function systemOperationsService(options = {}) {
  ...
  return {
    store,
    async today() {
      return decorateSnapshot(await store.getSystemOperationalSnapshot(), now());
    },
    async list(raw = {}) {
      ...
      const snapshot = decorateSnapshot(await store.getSystemOperationalSnapshot(), now());
      ...
      const orders = snapshot.orders.filter((order) => {
        ...
        if (normalized) return order.order_number === normalized;
    ...
    async detail(rawNumber) {
      ...
      const raw = await store.getSystemW1cOrderByNumber(orderNumber);
      if (!raw) return null;
    ...
    async transition(raw, actor) {
      ...
      const row = result.order || await store.getSystemW1cOrderByNumber(input.order_number);
...
module.exports = {
  systemOperationsService, normalizeSettings, slaFor, standardOverdue, nextActionFor,
