...
const INVOICE_PRODUCTION_STATES = new Set(['awaiting_processing', 'processing', 'ready']);
...
function invoiceFacts(order) {
  ...
  if (!INVOICE_PRODUCTION_STATES.has(order?.production_state)) {
...
function assertOwner(actor) {
  if (!actor || !['owner', 'manager'].includes(actor.role) || !clean(actor.actor_id, 120)) {
    throw new InvalidTransitionError('Owner authorization or Manager invoice authorization is required.');
  }
}
...
function systemInvoiceService(options = {}) {
  ...
  async function load(orderNumber) {
    ...
    let pricing = null;
    try { pricing = pricingFacts(order); }
    catch (error) {
    ...
    return { order, invoices, current, pricing, preview, blocker };
  }
  ...
  return {
    ...
    async context(orderNumber) {
      ...
      return {
        ...
        pricing:safeFacts(loaded.pricing), preview:safeFacts(loaded.preview), current_invoice:loaded.current,
...
module.exports = { systemInvoiceService, pricingFacts, invoiceFacts, safeInvoice, safeFacts, expectedVersion,
  IMMUTABLE_PAYMENT_STATES, INVOICE_PRODUCTION_STATES };
