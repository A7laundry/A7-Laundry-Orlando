  ...
  return {
    ...
    async review(raw, actor) {
      ...
      const result = await store.reviewSystemInvoice({
        order_number:normalizeOrderNumber(raw.order_number), expected_invoice_version:version,
        ...
        idempotency_key:actionKey('review', normalizeOrderNumber(raw.order_number), request),
    ...
    async void(raw, actor) {
      ...
      const result = await store.voidSystemInvoice({
        ...
        idempotency_key:actionKey('void', normalizeOrderNumber(raw.order_number), request),
        occurred_at:now().toISOString()
      });
      return { duplicate:Boolean(result.duplicate), invoice:safeInvoice(result.invoice) };
    }
