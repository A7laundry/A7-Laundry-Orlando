...
const { InvalidTransitionError, createOperationalStore } = require('./operational-store.js');
...
function etaInput(raw = {}) {
  const version = Number(raw.version);
  if (!Number.isSafeInteger(version) || version < 1) throw new InvalidTransitionError('Route version is invalid.');
  return { route_id:uuid(raw.route_id, 'Route identity'), stop_id:uuid(raw.stop_id, 'Stop identity'),
    eta_at:optionalTimestamp(raw.eta_at), version, request_id:requestId(raw.request_id) };
}
...
function pickupEligible(order) {
  return Boolean(order && !order.is_qa && order.order_status === 'pickup_scheduled'
...
function systemRouteService(options = {}) {
  const store = options.routeStore || createOperationalStore(options);
  ...
  return {
    async list(raw = {}, actor) {
      requireRouteActor(actor);
      return store.listRoutes({ route_date:raw.route_date ? dateOnly(raw.route_date) : null });
    },
    async detail(routeId, actor) { requireRouteActor(actor); return store.getRoute(uuid(routeId, 'Route identity')); },
    async eligible(raw = {}, actor) {
      requireRouteActor(actor);
      return store.listEligibleStops({ route_id:uuid(raw.route_id, 'Route identity') });
    },
    ...
    async removeStop(raw, actor) { requireRouteActor(actor); const input = { route_id:uuid(raw.route_id, 'Route identity'),
      stop_id:uuid(raw.stop_id, 'Stop identity'), request_id:requestId(raw.request_id) };
      return store.removeRouteStop({ ...input, ...actorFields(actor), idempotency_key:routeIdempotency('remove-stop', input) }); },
    ...
    async setEta(raw, actor) { requireRouteActor(actor); const input = etaInput(raw); return store.setRouteStopEta({ ...input, ...actorFields(actor), idempotency_key:routeIdempotency('set-eta', input) }); },
    ...
    async cancel(raw, actor) { requireRouteActor(actor); const input = routeActionInput(raw); return store.cancelRoute({ ...input, ...actorFields(actor), idempotency_key:routeIdempotency('cancel', input) }); }
  };
}

module.exports = { ROUTE_STATUSES, STOP_TYPES, STOP_STATUSES, EXCEPTION_REASONS, createRouteInput, addStopInput,
  reorderInput, etaInput, routeActionInput, stopActionInput, exceptionInput, pickupEligible, deliveryEligible, stopEligible,
