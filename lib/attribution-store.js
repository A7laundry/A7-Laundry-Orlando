...
function resolveAttributionSupabaseConfig(env = {}) {
  const candidates = [
...
function createAttributionStore(options = {}) {
  ...
  const supabase = resolveAttributionSupabaseConfig(env);
  if (explicitMemory || localMemory) {
    globalThis.__A7_ATTRIBUTION_STORE__ = new MemoryAttributionStore();
  } else if (supabase) {
    globalThis.__A7_ATTRIBUTION_STORE__ = new SupabaseAttributionStore(supabase);
  } else {
