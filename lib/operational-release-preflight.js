...
function evaluateOperationalRelease(env = {}, profile = '') {
  ...
  const checks = [
    ...
    check('stripe_secret_key', production ? stripeKey.startsWith('sk_live_') : stripeKey.startsWith('sk_test_'),
    ...
    check('memory_storage_forbidden', !production || text(env.A7_OPERATIONS_STORAGE_MODE) !== 'memory',
      'memory_forbidden_in_production')
  ];
