...
function nonNegativeCents(value, label = 'Amount') {
  ...
  if (!/^\d+(?:\.\d{1,2})?$/.test(raw)) throw new InvalidTransitionError(`${label} is invalid.`);
  const result = Math.round(Number(raw) * 100);
  if (!Number.isSafeInteger(result) || result < 0) throw new InvalidTransitionError(`${label} cannot be negative.`);
  return result;
}
...
function paymentInput(raw = {}) {
  ...
  const tipCents = nonNegativeCents(raw.tip_amount ?? '0', 'Tip');
  const reference = clean(raw.reference, 120) || null;
  if (['stripe', 'zelle'].includes(method) && !reference) {
    throw new InvalidTransitionError('Payment reference is required for Stripe or Zelle.');
  }
  return {
  ...
  return {
    ...
    async registerPayment(raw, actor) {
      ...
      return store.recordSystemManualPayment({
        ...
        tip_amount:input.tip_cents / 100,
        actor_id:actor.actor_id,
        actor_role:actor.role,
