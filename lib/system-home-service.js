...
function safeAction(order) {
  return {
    ...
    next_action:{ code:order.next_action?.code || 'operational_blocker',
      label:order.next_action?.label || 'AÇÃO OPERACIONAL INDISPONÍVEL' }
  };
}
...
function buildHomeProjection({ operational, financeToday = null, financeCurrent = null, financePrevious = null, actor, now = new Date() }) {
  if (!operational || !actor || !['owner', 'manager', 'operator'].includes(actor.role)) {
  ...
  const expressRisk = active.filter((order) => isInQueue(order, 'express_risk', {}));
  const expressLate = active.filter((order) => isInQueue(order, 'express_late', {}));
  ...
  for (const lead of waitingLeads.slice(0, Math.max(0, 5 - nextActions.length))) {
    nextActions.push({ kind:'lead', lead_ref:lead.lead_ref || null,
      customer_name:lead.customer_name, property:lead.property || null,
  ...
  const needsAttention = [
    ...
    { key:'express_risk', label:'Express em risco', count:expressRisk.length,
      tone:'amber', target:{ view:'orders', queue:'express_risk' } },
    { key:'express_late', label:'Express atrasado', count:expressLate.length,
      tone:'red', target:{ view:'orders', queue:'express_late' } },
  ...
  const home = {
    meta:{ as_of:operational.as_of, timezone, date:today, role:actor.role,
      sources:['operational_snapshot'], availability:['owner', 'manager'].includes(actor.role) && !financeToday ? 'partial' : 'current' },
    operation:{
...
function systemHomeService(options = {}) {
  ...
  return {
    async report(actor) {
      if (!actor || !['owner', 'manager', 'operator'].includes(actor.role)) throw new InvalidTransitionError('Home authorization is required.');
